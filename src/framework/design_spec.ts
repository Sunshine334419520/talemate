...
export const DESIGN_SPECS: Record<LayerId, DesignSpec> = {
  ...
  characters: {
    ...
    guide: [
      ...
      "工作法：",
      "1) 创建与修改都走 propose-design 带 name:\"characters/<名>.md\"（两段式，用户看过才落盘）。**没有专用的角色工具**——改某一格用 section:\"<格名>\"，卡上的格就是 `###` 标题；",
      "2) 先 read-design 看当前卡再动手；section 级提案只换那一格，其余小节（含自由长尾）字节不动；",
