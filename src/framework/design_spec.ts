...
export const DESIGN_SPECS: Record<LayerId, DesignSpec> = {
  ...
  characters: {
    ...
    // 名单（有哪些人、常驻带齐没齐）由 list-designs 现算，没有派生文件。
    sections: [],
    guide: [
      "一角色一卡 characters/<名>.md = `# 角色：<名>` + 若干 `###` 小节，分两层：",
      "  常驻带（写 ta 的任何一场戏都要带，先填这四格）：### 基本档案（逐行「键：值」：姓名 / 性别 / 年龄段 / 身份 · 所属 / 出身，例：`性别：男`；名单取「身份 · 所属」那一行）/ ### 想要 · 最怕 / ### 底线 · 绝不做 / ### 说话方式（给一句『声音范例』原文，别只写性格形容词）；",
      "  按需格（按场景切片，按戏份随时补，不填不是缺陷）：### 性格与矛盾 / ### 来历 · 成因 / ### 语录 / ### 身体 · 习惯 / ### 关联角色 / ### 能力 · 机制 / ### 转变 · 走向 / ### 在故事中的功能。",
      "  `### 当前`（在场/已故 + 此刻处境）**归这一格，别写进基本档案**；update-character 不碰它，要改它用 propose-design 带 section:\"当前\"（两段式，用户过目后才落盘）；规范外的自定义 `###` 小节允许（世界特有的，如「回响」），工具会原样保留。",
      "工作法：",
      "1) 加角色用 add-character：缺的常驻格写（待定）并提示补；别逐格盘问——让用户用自己的话讲；",
      ...
      "5) 一角色一卡；名单（有哪些人、常驻带齐没齐）由 list-designs 现算，没有需要手改的总表；删角色用 remove-character。",
    ].join("\n"),
  },
  outline: {
    id: "outline",
...
export function renderDesignSpec(id: LayerId): string {
  ...
  return [
    ...
    isDir
      ? "本层不是单个文档：一角色一卡，形状见下（名单——有哪些人、常驻带齐没齐——由 list-designs 现算）。"
