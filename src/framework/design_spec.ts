...
const WORK_METHOD = [
  ...
  "3) 用 propose-design 把草稿摆给用户看（**要先在草稿模式里**，三向审阅只有那一条通道）——**这一步不写盘**，本回合到此为止，等他回话：",
...
export const SPECS: DocSpec[] = [
  ...
  // 势力归 characters；悬念/未解之谜归 outline「伏笔与回收登记」。
  {
    ...
    guide: [
      ...
      "3) 先让用户用自己的话讲 → 按小节整理成草稿（没讲到的写（待定））→ propose-design 摆给他看（**要先在草稿模式里**）、等回话 → 认可后 apply-design；",
  ...
  // 文档"），而不是回一句误导的"未登记"。卡的形状在 guide 里（卡内是 ###），所以 sections 为空。
  {
    ...
    guide: [
      ...
      "5) 一角色一卡；名单（有哪些人、必有格齐没齐、卡上还有哪些自由小节）由 list 现算，没有需要手改的总表；删角色用 delete，它会先把「这个名字还在哪儿出现」摆出来，级联清理你自己用 edit 做。",
    ].join("\n"),
  },
  ...
  // （match 以 `/` 结尾、没有 target），真正的形状在下面按路径登记的两行里，与 characters 同构。
  {
    ...
    guide: [
      ...
      "propose-design 摆给他看、本回合到此为止 → 认可后 apply-design。文档一旦建立，后续以文件当前内容为准（先 read 再动）。",
    ].join("\n"),
  },
  ...
  // **不靠表序**——所以这一行放在卷纲前面还是后面都一样（见 `specFor`）。
  {
    ...
    guide: [
      ...
      "",
      "工作法：先让用户用自己的话讲 → 整段写成草稿 → propose-design 摆给他看（**要先在草稿模式里**）、",
