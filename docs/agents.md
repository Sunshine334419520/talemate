...
| `delete` | 删掉**整份文件**。**删前把"这个名字还在哪儿出现"放进 confirm**——影响面不在被删的那份文件里 |
...
**`delete` 不做级联。** 它把引用列出来，清理由模型用 `edit` 逐处做——"该不该动 `world.md` 里那句话"是判断，不是机械操作。opencode 那边没有 delete 工具（删除折在 `apply_patch` 里），它能那样做是因为它还有 `bash`；talemate 把 shell 剥掉了，所以删除必须是一个工具。
...
| id | 用途 |
|---|---|
| `propose-design` | 把提案交给用户看，**不写盘**，并结束本回合 |
...
1. **企划对话（含大纲）**：mate 直接答；查 = `list`/`read`/`search`。
   ...
   - **删整份文档**（角色卡、过期专题页）= `delete`：它先把引用列出来，**级联清理由 mate 自己用 `edit` 做**。
