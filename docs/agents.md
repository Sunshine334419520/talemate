...
3. **一个任务的"流程"不单独成角色。** "规划→写正文→拍板"是 editor 的工作协议，不是新 Agent。
...
① 写一个 `AgentDef`（含 system）→ ② 决定 tools 白名单 → ③ 决定触发：被 editor 委派就 `mode:"subagent"`（task 的可见列表由运行时自动带出），是新对话面才考虑 `mode:"primary"`。
...
> ⚠️ `AgentDef.tools` 只决定 `schemasFor` 喂给模型**哪些 schema**（"广告"）；执行时查的是**全局** registry（`runner.ts` 按 id 查表）。
>
> **撤下一个工具必须真删定义**（从 `DESIGN_TOOLS` 等数组里移除），只从白名单拿掉等于没拿掉。
...
- `halt`：工具在**成功**时可要求结束本回合（目前只有 `propose-design`）。
...
- **`confirm`**：写/覆盖活文档、落盘正文等**不可轻易反悔**的动作前，展示"要做什么 + 影响面 + 内容"，用户拍板。**不需要它时**工具可声明 `confirm:false`——只给"内容已经过提案回合、用户已过目"的入口（`apply-design`）。
- **`ask-user`**：agent 需要用户出想法/裁决时用（"主角的金手指是什么？"）。**不是权限审批，是参谋要决策。**

权限模型极简：**默认全放行 + 每个工具自声明是否需 confirm**，不搬 allow/ask/deny 级联。
