...
**工具那一半已经定了**：边界靠**一条权限规则**，不靠专用工具。writer 拿通用的 `write`，范围由
`edit: {"design/*": "deny"}` 划死——于是"加一个要保护的目录"是加一条规则，不是造一个工具。

**谁落盘那一半仍然是个取舍**，两条路二选一，不能都留：

- **writer 不落盘**：只回传正文文本，mate 收到后落盘，confirm 落在 mate 层。代价：长章节整篇进 mate 上下文。
- **writer 落盘**（当前）：长文直接落盘、只回摘要，省 mate 上下文；但 confirm 在子会话里弹，mate 对成品的把关变弱。

折中落点（未采纳）：writer 落盘 + 回传短摘要 + 文件路径。
