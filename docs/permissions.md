...
**第一步是本设计的关键。** 只做第二步（顺序即一切，用户配置最强）的话，别处的一条 `allow` 就能盖掉这里的 `deny`。所以额外让 `deny` 单调：**"不许"不该被别处的"允许"盖掉。**

这不是新发明——**子代理派生那里用的就是同一条原则**（父的 `deny` 继承、父的 `allow` 不继承）。这里只是把它推广到求值。
...
**`detail` 是给人做判断的材料——传的是 diff。** 从前传的是**完整内容**；改成 diff 是因为局部修改要看的是"改了哪一段、落在哪"，而全文恰恰看不出这一点（还吵）。
