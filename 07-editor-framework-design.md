...
>
> ⚠️ **2026-09-06 修订（以本段为准，旧章节按需读）**：企划层 **懒建**（createProject 不播种；docs 空，文件在用户点名某层后才被写入）；**不再自动注入 DESIGN_PROTOCOL**——用户说「完善核心设定」等时，editor 调 `design-spec` 工具拿该层结构，**按用户自由描述成稿（不逐格盘问）→ 确认后整层落盘 → 之后只 edit-design 还待定/要改的小节**。进入空间只展示四层现状卡片 + 提示，干什么由用户决定。
...
| **框架维护** | docs 是你的活文档：查 = list-designs / read-design / search-designs；增 = append-design；改 = edit-design；删 = remove-design-section；整篇重写才用 write-design。动前先 read 当前版；confirm = 用户拍板，别绕过 |
...
| 工具 | 何时用 | confirm |
|---|---|---|
| `list-designs` | 开场/不确定有哪些材料；返回各文档 + 小节标题（含未填 `（待定）` 可见） | — |
| `read-design` | 读整篇；或带 `section`（小节标题，如「主角」「角色：沈越」）只读一格 | — |
| `design-spec` | 用户点名完善某层时拿该层"结构规范 + 成稿做法"（懒建：docs 平时不存在） | — |
| `search-designs` | 改/删某个设定前查它被谁引用（影响面） | — |
| `append-design` | 追加一块（新角色卡 / 新设定小节）；block 自带 `##` 标题，重名会提示改用 edit | 无（非破坏） |
| `edit-design` | 改一个小节正文（其余原样保留） | ✅ 动态（旧→新长度） |
| `remove-design-section` | 删一个小节 | ✅ 动态（含引用检查命中） |
| `write-design` | 整篇重写（脚手架 / 整体重构） | ✅ |
...
editor 只有一个人格；进入空间**只展示四层现状卡片**（`src/framework/report.ts`：有/无、是否待完善），**干什么由用户决定**。每次请求 editor 都带 core/world 常驻设定（§5）；**设计引导不自动注入**——用户点名要完善某层时，editor 调 `design-spec` 工具（`src/framework/design_spec.ts`）按需拿该层结构规范再成稿。

---

## 2. 小说框架模型 —— 四层 DocKind

结构规范是**数据**（`src/framework/design_spec.ts`：core/world/characters/outline 各该有哪些小节 + 成稿/补缺做法）；**懒建**——文件平时不存在，用户点名某层时 editor 用 `design-spec` 拿形状、按其小节成稿落盘。文件一旦建立即内容与真相，之后 `read-design` 为准。
...
### 2.1 各层目标结构（design_spec 数据定义；`（待定：…）` = 未填格）
...
<!-- 加角色：用 append-design 追加以下结构，把 〈〉 换成实际名字
## 角色：〈名字〉
### 一句话定位
### 想要 · 最怕
...
flowchart TD
    ...
    B -- 是 --> C[调 design-spec 拿该层结构规范 <br/>core / world / characters / outline]
    C --> D[按小节把用户自由描述整成草稿<br/>没讲到的写（待定）]
    D --> E[write-design 整层落盘 → confirm 拍板]
    E --> F{这层还有关键格(待定/要改)?}
    F -- 是 --> G[edit-design 那一节 → 给建议/选项, 一次可多答]
    G --> F
    F -- 否 --> H[向用户报一句：当前已定 + 还待定]
    H --> I{用户冒出新点子 / 推翻旧设定?}
    I -- 是 --> J[判定归哪层; 牵动别处用 search-designs 查影响面;<br/>小改自己 edit, 大级联 task planner]
    J --> H
...
2. **点谁做谁**：editor **不自动注入设计协议**——用户点名要完善某层时，editor 调 `design-spec` 拿该层结构规范再成稿（懒建：docs 平时不存在，写了才出现）。
3. **聊定才写**：没写进 docs 的不算已定。成稿先按小节把用户的话整理（缺的写（待定））→ `write-design` 整层落盘 `confirm` 拍板；之后只改仍（待定）/要改的那一小节（`edit-design`），补细节给建议/选项、一次可答多个。
4. **新点子即时归类**：当场判断归哪层，聊定落盘；推翻旧设定先用 `search-designs` 看影响面，小改自己 `edit-design`，牵动多层的结构级改动派 planner（`task(planner)`，让它 read 全量 docs 产出一致的新版，editor 复核给用户）。
...
- **confirm**：`edit-design`/`remove-design-section`/`write-design` 在 execute 内做（给旧→新摘要 / 引用命中），拒绝返回文案不落盘。
- **删前查引用**：`remove-design-section` 收到 `term`（实体名）→ 内置 `search-designs(term)`，命中列进 confirm 摘要——**强制点**，模型想跳也跳不过。
- **常驻设定不用维护**：core/world 由 `buildResidentDocs` 每轮**现读**注入，永不过期，无写后刷新钩子。

各工具的执行语义与返回见 `src/tool/（按领域模块：design_tools / character_tools / framework_tools / core_tools（工具 id 用 kebab））`。

---

## 5. 常驻设定与上下文组装

editor（可见 primary）每次请求的 system = `env + 角色 system + core/world 常驻设定 + AGENTS.md + skill 目录`（`src/context/assemble.ts` / `src/session/session.ts`；设计协议不再注入，结构规范由 `design-spec` 工具按需给出）：
...
- **core + world 常驻 = 设定全文，不带"状态"包装**：core（小说介绍）是写作方向不变量、world（舞台与规则）是场景规则，任何一章都得对它们 → `buildResidentDocs` 每轮现读注入（2026-09-06 追加：世界观随 core 一起常驻，见 06 §8）。原 `<nvl-state>` 块已删——它名义上是状态锚点、实际却搬运整份文档 + 进度 + 索引（名实不符）。写作进度/伏笔/待定等**状态**不属于静态基线：归 list-chapters / list-designs 工具（及未来的独立状态块），characters/outline 全文仍按需 read-design。
- **只给可见 primary（editor）注入**；subagent 不注入（省 token，靠 task prompt 切片 + 自己 read-design 补齐）。

---

## 6. 跑通示例：从"一个念头"到框架（对照真实工具）

> 前置：`talemate novel create "荒岛"` 进入主编会话（docs 为空，懒建）。进入先看到四层现状卡片 + 提示；以下示例是"用户点名完善核心设定"后 editor 的成稿流程（先吸收自由描述 → 按 design-spec 小节成稿 → 确认落盘 → 只补缺）。

**用户**：想写个荒岛求生的故事，一个男人和一个空姐流落荒岛。

`[editor: 每轮注入 persona + core/world 常驻设定。用户说『完善核心设定』→ editor 调 design-spec(core) 拿结构，按小节把用户的话整理成稿]`
...
```
# 一句话简介  ← edit-design(core.md, "一句话简介", "空难后，习惯性扛责任的普通人沈越和空姐被困荒岛，没有系统没有外挂，只有脑子与自然你死我活——他想带她活着回去。")
```
...
**editor**（继续 core）：
```
# edit-design(core.md, "题材 · 频道", "双频｜求生向，感情为暗线")
# edit-design(core.md, "基调 · 情绪", "紧绷求生的紧张 + 相依为命的暖；每章兑现'又撑过一劫'的释放")
```
...
**editor**:
```
# edit-design(core.md, "金手指 / 超常设定", "无超自然。靠知识/体力/协作求生。边界：仍受真实约束，越界=违背本书基调")
# append-design(world.md, "## 空间与舞台\n无人礁岛，亚热带，淡水溪+山洞。岛上前人没走成的遗迹——仍生效的过去作成因写这，悬念引爆归 outline 伏笔登记")
```
...
**editor**:
```
# append-design(characters.md, "## 角色：林晚\n\n### 一句话定位\n空姐，与沈越困同一座岛。\n\n### 想要 · 最怕\n想要体面地活着回去；最怕成为拖累。\n\n### 说话方式\n嘴硬心软，关心总是反着说；口头禅式原话：\"你死了我可不会埋你\"。\n\n### 习惯动作\n紧张时数身边的东西够不够用。\n\n### 在故事中的功能\n沈越的对照与软肋，感情暗线。")
```
……
...
```
# task(planner, { prompt: "综合 core/world/characters（自己 read-design），按 outline 骨架产出：一句话主线 + 前3章钩子 + 分卷方向。不写正文。" })
```
**planner**（回传 `<task_result>` 一段主线 + 前3章钩子 + 分卷草案）→ editor 展示给用户 → 逐项拍板后 `edit-design(outline.md, …)` 落盘 → `designActive` 变 false，设计协议退场，项目进入写作段就绪。
...
- 结构规范与懒建：`src/framework/design_spec.ts`（按需取规范）· `src/storage/project.ts`（createProject 不播种）· `design-spec`/`add-character` 工具
...
- 框架工具集：`src/tool/（按领域模块：design_tools / character_tools / framework_tools / core_tools（工具 id 用 kebab））` · task 动态目录：`src/tool/registry.ts`
